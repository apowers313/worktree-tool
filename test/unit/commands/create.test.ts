...
describe('Create Command', () => {
  ...
  describe('validateCreateOptions', () => {
    it('should accept valid options', () => {
      expect(() => validateCreateOptions({ name: 'feature-branch' })).not.toThrow();
      expect(() => validateCreateOptions({ name: 'my feature' })).not.toThrow();
    });

    it('should reject empty name', () => {
      expect(() => validateCreateOptions({ name: '' })).toThrow(ValidationError);
      expect(() => validateCreateOptions({ name: '' })).toThrow('Worktree name is required');
    });
    ...
    it('should reject names with only invalid characters', () => {
      expect(() => validateCreateOptions({ name: '~^:?*[]\\!' })).toThrow(ValidationError);
      expect(() => validateCreateOptions({ name: '~^:?*[]\\!' })).toThrow('contains only invalid characters');
    });

    it('should reject very long names', () => {
      const longName = 'a'.repeat(150);
      expect(() => validateCreateOptions({ name: longName })).toThrow(ValidationError);
      expect(() => validateCreateOptions({ name: longName })).toThrow('too long');
    });
  });
